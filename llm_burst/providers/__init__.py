...
def _build_chatgpt_injector(
    ...
    async def _chatgpt_inject(page: Page, prompt: str, opts: "InjectOptions") -> None:
        ...
        if use_paste:
            ...
            # Verify research mode activation for ChatGPT
            if opts.research:
                await asyncio.sleep(0.5)  # Small delay to let UI update
                verification_script = """
                    () => {
                        // Check if we're on Research Prompts GPT
                        const inResearchGPT = window.location.href.includes('research-prompts');
                ...
                try:
                    result = await page.evaluate(verification_script)
                    if result.get("inResearchGPT"):
                        print("✓ Research mode active: Using Research Prompts GPT")
                    elif result.get("hasIndicators"):
                        print("✓ Research mode active: Deep research enabled")
                    else:
                        print("⚠ Research mode verification pending")
                except Exception:
                    pass  # Silent fail on verification
