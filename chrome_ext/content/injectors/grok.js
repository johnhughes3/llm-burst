/**
 * LLM Burst - Grok Injector
 * Ported from llm_burst/sites/grok.py (SUBMIT_JS and FOLLOWUP_JS), adapted for MV3 content script.
 *
 * Exposes:
 *   window.llmBurst.injectors.GROK = {
...
(function() {
  ...
  // ---------------------------------------------------------------------------
  //  INITIAL SUBMIT (first prompt)  - Ported SUBMIT_JS
  // ---------------------------------------------------------------------------
  ...
  async function executeTextInput(inputElement, text) {
    console.log(`Inputting text (${(String(text || '')).length} chars)...`);
    ...
    try {
      if (isContentEditable) {
        ...
        try { inputElement.value = String(text || ''); } catch {}
        ...
        try {
          ...
          try { inputElement.dispatchEvent(new Event('input', { bubbles: true, cancelable: true })); } catch {}
        }
        ...
        if (attempts >= maxAttempts) {
          if (!isContentEditable) {
            ...
            try {
              ...
              await wait(100);
              if (inputElement.value === String(expectedText || '')) {
  ...
  async function clickSubmitButton(inputElement) {
    ...
    try {
      ...
      const submitButton = await waitUntil(() => {
        ...
        return null;
      }, 8000, 200); // Increased timeout for reliability
  ...
  window.simulateFocusSequence = function(element) {
    ...
    events.forEach(event => element.dispatchEvent(event));
    try { element.focus(); } catch { /* ignore */ } // Actual browser focus
  }
  ...
  // ---------------------------------------------------------------------------
  //  FOLLOW-UP MESSAGE  - Ported FOLLOWUP_JS
  // ---------------------------------------------------------------------------
  ...
  // === Compatibility aliases (added by llm-burst) =============================
  // We keep the same aliases as submit section for consistency
  // const wait      = (...args) => window.llmBurstWait(...args);
  // const waitUntil = (...args) => window.llmBurstWaitUntil(...args);
  // =============================================================================
  ...
  async function performFollowUp(messageText, debug) {
    ...
    try {
      ...
      log(`Inputting text (${String(messageText || '').length} chars)...`);
  ...
  async function inputTextToTextarea(textarea, text, log) {
    ...
    try { simulateTypingActivity(textarea); } catch {}
  ...
  async function verifyTextAccepted(textarea, expectedText, log) {
    return new Promise(resolve => {
      ...
      const checkAccepted = () => {
        ...
        const valueCorrect = (textarea.value || '') === String(expectedText || '');
  ...
  async function executeInputFallback(textarea, text, log) {
    ...
    try {
      const execCommandSupported = document.queryCommandSupported && document.queryCommandSupported('insertText');
    ...
    [
      ...
      new InputEvent('input', { bubbles: true, cancelable: true, inputType: 'insertFromPaste', data: String(text || '') }),
      new Event('input', { bubbles: true, cancelable: true }),
      new Event('change', { bubbles: true, cancelable: true })
    ].forEach(event => {
      try { textarea.dispatchEvent(event); } catch {}
    });

    // Simulate keystrokes to activate listeners
    'abcdefghij'.split('').forEach(char => {
      try { textarea.dispatchEvent(new KeyboardEvent('keydown', { key: char, bubbles: true })); } catch {}
      try { textarea.dispatchEvent(new KeyboardEvent('keyup', { key: char, bubbles: true })); } catch {}
    });
  ...
  async function clickSubmitButtonFollowup(button, log) {
    ...
    for (const event of events) {
      try { button.dispatchEvent(event); } catch {}
  ...
  try { console.debug('LLM Burst Grok injector loaded'); } catch {}
})();
