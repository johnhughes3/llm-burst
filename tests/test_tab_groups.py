...
def test_tab_groups_sync_functions():
    ...
    from llm_burst.tab_groups import create_group_sync, move_to_group_sync, list_groups_sync
        ...
        with patch('llm_burst.cli_click.move_to_group_sync') as mock_move:
            with patch('llm_burst.state.StateManager') as MockState:
                with patch('llm_burst.cli_click.send_prompt_sync'):
                    with patch('llm_burst.cli_click.prompt_user'):  # Mock prompt_user to prevent swiftDialog call
                        # Setup mocks
                        handle = Mock()
                        ...
                        result = runner.invoke(
                            cli,
                            ["open", "-p", "gemini", "-t", "Test-Task", "--group", "Research"]
                        )
